 ...
 */
function weigh(property: Property, element: Element, style: CSSStyleDeclaration, rect: DOMRect): number {
...
export function scanColors(root?: ParentNode): ScannedColor[] {
  ...
  const elements = [target, ...target.querySelectorAll('*')].filter((node): node is Element => node instanceof Element);
