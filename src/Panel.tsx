...
export const Panel: React.FC<{ active: boolean }> = ({ active }) => {
	...
	const ratio =
		dominant && surface && text && surface !== text
			? (contrastRatio(surface.hex, text.hex) as number)
			: null;
	...
	return (
		<AddonPanel active={active}>
			<div style={{ padding: 15 }}>
				...
				{result?.error && (
					<Placeholder>Could not read the story: {result.error}</Placeholder>
				)}
				...
				{result && !result.error && colors.length === 0 && (
					<Placeholder>
						This story renders no colors of its own. Everything it shows is
						inherited or transparent.
					</Placeholder>
				)}
					...
					<>
								...
								{colors.slice(0, 12).map((entry) => {
									...
									return (
										<tr key={entry.hex}>
											...
											<td style={{ padding: '4px 8px 4px 0', fontFamily: 'monospace' }}>
												{clean(entry.hex)}
											</td>
											<td
												style={{
											...
											>
												oklch({(oklch.L * 100).toFixed(1)}% {oklch.C.toFixed(3)}{' '}
												{oklch.h.toFixed(1)})
											</td>
											<td style={{ padding: '4px 8px 4px 0', opacity: 0.75 }}>
												{share < 1 ? '<1' : share.toFixed(0)}%
											</td>
											<td style={{ padding: '4px 0', opacity: 0.6, fontSize: 11 }}>
												{entry.sources.join(', ')}
											</td>
										</tr>
									);
						...
						{ratio != null && surface && text && (
							<p style={{ marginTop: 14, opacity: 0.8 }}>
								The surface this component paints is {clean(surface.hex)} and the
								main text color on it is {clean(text.hex)}, measuring{' '}
						...
						{ratio == null && (
							<p style={{ marginTop: 14, opacity: 0.8 }}>
								This component paints no background of its own, so its contrast
								depends on wherever it is placed. The a11y addon checks each
								element against the surface it actually sits on.
							</p>
						)}

						<p style={{ marginTop: 10, opacity: 0.6, fontSize: 11 }}>
							Percentages are share of painted area, so a full-width surface
							outranks a hairline border. Text colors are weighted by the ink of
							the glyphs rather than the box around them.
						</p>
					</>
				)}
			</div>
